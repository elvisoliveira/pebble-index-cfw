...
void user_catch_rest_hndl(ke_msg_id_t const msgid, void const *param, ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    if (msgid == CUSTS1_VAL_WRITE_IND) {
         ...
         * itself. clip_tx needs it because sending unsubscribed silently succeeds. */
        else if (p->handle == SVC1_IDX_AUDIO_NTF_CFG && p->length >= 1)
            clip_tx_set_subscribed((p->value[0] & 0x01) != 0);
    }
